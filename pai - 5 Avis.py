...
def raport2():
    ...
    for element1 in contracte_cu_pai_nevandut:
        scriere_log("Pe contractul "+element1.contract+" deschis pe "+element1.outlocT+" s-a vandut PAI, dar nu s-a completat polita\n")
        if element1.tarif1_out=="6FI":
            rata_6F=rata_6F+1
        if element1.grp_crg=="L":
            incasare_Avis=float(incasare_Avis)+float(element1.pai_amt)
            cedare_Omniasig=float(cedare_Omniasig)+float(3.4)*float(element1.LOR)
        else:
            incasare_Avis=float(incasare_Avis)+float(element1.pai_amt)
            cedare_Omniasig=float(cedare_Omniasig)+float(2)*float(element1.LOR)
    print "\nNoi am incasat "+str(incasare_Avis)+" EUR si ar fi trebuit sa cedam catre Omniasig "+str(cedare_Omniasig)+" EUR. Au fost "+str(rata_6F)+" contracte pe rata 6F in care PAI-ul este inclus in tarif si nu se taxeaza suplimentar."
...
def raport4():
    """Polite PAI raportate ca vandute, care nu figureaza in lista de serii PAI primite de la Omniasig"""
    lista_temp=[]
            ...
            if element1.contract==element2.contract and element1.raportat!="Corectii":
                ...
                if element1.LOR!=element2.LOR:
                    ...
                    if diferenta_zile>0:
                        ...
                        diferenta_suma=diferenta_suma+temp
    if diferenta_suma>0:
        scriere_log("\nPentru diferentele de sume de mai sus, avem de luat de la Omniasig "+str(diferenta_suma).strip("-")+ " EUR\n")
    elif diferenta_suma<0:
        scriere_log("\nPentru diferentele de sume de mai sus, avem de dat catre Omniasig "+str(diferenta_suma).strip("-")+ " EUR\n")
    scriere_log("\n")
